...
JDK_BOOL JDK_get_tv_adjust(JDK_INT32 * x, JDK_INT32 * y, JDK_INT32 * w, JDK_INT32 * h)
{
	if(!x || !y || !w || !h) {
		return JDK_FALSE;
	}
...
#define __ALLIGNMENT_BY(x, y) {(x) = (x) - (x)%(y);}
	...
	cvbs_x = x; cvbs_y = y; cvbs_w = w; cvbs_h = h;
	...
	JDK_UINT32 nCVBSOffsetX, JDK_UINT32 nCVBSOffsetY, JDK_UINT32 nCVBSWidth, JDK_UINT32 nCVBSHeight, JDK_UINT32 nCVBSMode)
{
	...
	if (monitors & JDK_MONITOR_CVBS) {
		...
		{
			JDK_INT32 x, y, w, h;
			if(JDK_TRUE == JDK_get_tv_adjust(&x, &y, &w, &h)) {
				jdk_return_val_if_fail(JDK_set_tv_adjust(x, y, w, h, JDK_TRUE), JDK_FALSE);
			}
		}
	}
...
static JDK_BOOL JDK_show_user_pic(void)
{
	...
	nPageChnBase = VO_GRID_BASE_GRID_OF_PAGE(s_jdkCtx->nCurDivision, s_jdkCtx->nCurPage);
...
JDK_BOOL JDK_set_vo_division(VO_DIVISION_TYPE div, JDK_UINT32 nPage)
{
	...
	nPageChnBase = VO_GRID_BASE_GRID_OF_PAGE(div, nPage);
...
JDK_BOOL JDK_swap_vo_chn(JDK_UINT32 chn1, JDK_UINT32 chn2)
{
	...
	// 2>  close vo chn
	if (s_jdkCtx->voIface->close_chn(s_jdkCtx->nVoDevList[nDev],  npageBase + s_jdkCtx->voChnMap[chn1]) == JDK_FALSE) 
	{
		JDK_TRACE("hide chn:%u false", npageBase + s_jdkCtx->voChnMap[chn1]);
	}
	if (s_jdkCtx->voIface->close_chn(s_jdkCtx->nVoDevList[nDev],  npageBase + s_jdkCtx->voChnMap[chn2]) == JDK_FALSE) 
	{
...
JDK_BOOL JDK_vin_init(JDK_BOOL viInModeAuto)
{
	...
	for ( i = 0; i < JDK_BNC_CAM(); i++) {
		...
		arg.rect.x = 0;
		arg.rect.y = 0;
		arg.inMode = JDK_INPUT_BNC_720P_25;
		arg.fps = 25;
		arg.flip = JDK_FALSE;
	...
	if (chn < JDK_ACTIVE_CAM()) {
		if ((JDK_IS_BNC_CAM(chn) == JDK_TRUE)) {
			if (1) {
				...
				if ((arg->inMode == JDK_INPUT_BNC_HFHD_25) || (arg->inMode == JDK_INPUT_BNC_HFHD_30)) {
