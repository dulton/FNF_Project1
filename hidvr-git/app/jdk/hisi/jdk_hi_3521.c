...
#if defined(_EXT_HDDVR_LITE)
static JDK_UINT32 VIMAP_TBL_8[8] = { 8, 9, 12, 13, 0, 1, 4, 5, };
#else
static JDK_UINT32 VIMAP_TBL_8[8] = { 12, 14, 0, 2, 8, 10, 4, 6,};
#endif

static JDK_UINT32 VIMAP_TBL_16[16] = { 14, 15, 12, 13, 2, 3, 0, 1, 10, 11, 8, 9, 6, 7, 4, 5, };
    ...
    int  NeedDie, int NeedIE, int NeedSp)
{
    {  //Greate VPSS Group;
        ...
        stGrpAttr.bIeEn     = NeedIE ?HI_FALSE :HI_FALSE;
        stGrpAttr.bNrEn     = NeedIE ?HI_FALSE :HI_FALSE;
    ...
    if(NeedIE) {
		...
		if (JDK_SOC_CHIP_IS(JDK_HI_SOC_3521)) {
	        ...
	        tmpGrpParam.u32IeStrength = 0;
	        tmpGrpParam.u32IeSharp    = 16;
	        tmpGrpParam.u32SfStrength = 16;
	        tmpGrpParam.u32TfStrength = 12;
		}
...
JDK_BOOL hi_3521_vi_chn_init(int Chnl, lpJDK_VIN_ARG arg, HD_INPUT_FORMAT Frmt)
{
	...
	tmpSrcSizW = tmpRefSizW;
	tmpSrcSizH = tmpRefSizH;
	if((Frmt == HDVIDEO_SD960H25FPS) || (Frmt == HDVIDEO_SD960H30FPS)) {
		tmpSrcSizW *= 2;
	}
	printf("\n%s : %d, %d\n", __FUNCTION__, tmpRefSizW, tmpRefSizH);
...

    {
...
#else
			if((0 != ((tmpRealChnl/4)%2)) && (tmpRefSizW >= JDK_VRES_SIZE_W_HD1080P)) {
	            tmpPicFmt = PIXEL_FORMAT_YUV_SEMIPLANAR_422;
	        }
#endif
		} else if (JDK_SOC_CHIP_IS(JDK_HI_SOC_3531)){
...
#else
	        if((0 != ((tmpRealChnl/2)%2)) && (tmpRefSizW >= JDK_VRES_SIZE_W_HD1080P)) {
	            tmpPicFmt = PIXEL_FORMAT_YUV_SEMIPLANAR_422;
	        }
#endif
		}
        ...
        tmpChnAttr.stCapRect.u32Width   = tmpSrcSizW;
        tmpChnAttr.stCapRect.u32Height  = tmpSrcSizH;
...
static JDK_BOOL hi_3521_bnc_vi_open(JDK_UINT32 chn, lpJDK_VIN_ARG arg)
{
	...
	// 1, init vi dev
	switch(arg->inMode) {
		case JDK_INPUT_BNC_D1_25:
		case JDK_INPUT_BNC_720H_25:
			viResol = HDVIDEO_SD720H25FPS;
			break;
		case JDK_INPUT_BNC_D1_30:
		case JDK_INPUT_BNC_720H_30:
			viResol = HDVIDEO_SD720H30FPS;
			break;
		case JDK_INPUT_BNC_960H_25:
			viResol = HDVIDEO_SD960H25FPS;
			break;
		case JDK_INPUT_BNC_960H_30:
			viResol = HDVIDEO_SD960H30FPS;
			break;
		case JDK_INPUT_BNC_720P_25:
			viResol = HDVIDEO_HD720P25FPS;
			break;
		case JDK_INPUT_BNC_720P_30:
			viResol = HDVIDEO_HD720P30FPS;
			break;
		case JDK_INPUT_BNC_720P_50:
			viResol = HDVIDEO_HD720P50FPS;
			break;
		case JDK_INPUT_BNC_720P_60:
			viResol = HDVIDEO_HD720P60FPS;
			break;
		case JDK_INPUT_BNC_HFHD_25:
		case JDK_INPUT_BNC_1080P_25:
			viResol = HDVIDEO_HD1080P25FPS;
			break;
		case JDK_INPUT_BNC_HFHD_30:
		case JDK_INPUT_BNC_1080P_30:
			viResol = HDVIDEO_HD1080P30FPS;
			break;
		case JDK_INPUT_MODE_NULL:
	...
	viResol = HDDVR_InputFormatCheck(chn);  //Get Real Video Format
	if (viResol == HDVIDEO_UNKNOWN) {
		viResol = HDVIDEO_HD720P25FPS;
	}
	...
	//if ((chn % arg->muxMode) == 0) { /* make sure the vi-device of all channels in a group init only once */
		hi_3521_vi_dev_init(viDev, arg, viResol);
	//}
...
static JDK_BOOL hi_3521_bnc_vi_digital_zoom_deinit(JDK_UINT32 nChn)
{
	if (JDK_IS_BNC_CAM(nChn)) {
		...
		// upbind vi->vpss2
		jdk_hi_return_val_if_fail( HI_MPI_Sys_UnBindByForce(HI_ID_VPSS, JDK_TOTAL_CAM()+ JDK_ACTIVE_CAM(), 0), JDK_FALSE);
