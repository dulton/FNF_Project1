...
#define GPIO_DEVICE_NAME "nc_vdec"
...
#define CMD_DETECT(cmd) (OPT_MATCH(argv[0],cmd))
	...
	while(fgets(szInput, sizeof(szInput) - 1, stdin)){
		...
		else if(CMD_DETECT("utc"))
		{
			#if 0
			int arg1 = atoi(argv[1]);
			int nFileD = open(GPIO_DEV, O_RDWR);
			if (nFileD < 0){
       			 printf("Open Dev nc_vdec Error!\n");
      			 return -1;
    		}
			printf("nFileD=%d,GPIO_DEV=%s\n", nFileD, GPIO_DEV);
			if(ioctl(nFileD, IOC_VDEC_PTZ_PELCO_SET, &arg1)<0){
				 printf("Call cmd IOC_VDEC_PTZ_PELCO_IRIS_OPEN fail\n");
            	 return -1;
			}
			close(nFileD);
			#endif
			#if 1
			HDDVR_UTC_Init(atoi(argv[1]));
			if(argc == 1 || argc == 2){
				return 0;
			}
			else{
				if(argc >= 4){
					if(atoi(argv[2]) == 0 || atoi(argv[2]) == 1){
						for(j=0; j<atoi(argv[3]); j++){							
								HDDVR_UTC_Send(atoi(argv[1]), atoi(argv[2]), NULL);
								usleep(35*1000);							
						}
					}
					else{
