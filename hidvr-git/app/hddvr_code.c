...
int HDDVR_AudioLiveLoop(int Chnl)
{
	...
	if(gHDChip == HDCHIP_HDTVI) {
		return HDTVI_AudioLiveLoop(Chnl);
	}
...
int HDDVR_AudioVolume(int Chnl, int Vol)
{
	...
	if(gHDChip == HDCHIP_HDTVI) {
		return HDTVI_AudioVolume(Chnl, Vol);
	}
...
int HDDVR_AudioPlayBack(int Chnl)
{
	...
	if(gHDChip == HDCHIP_HDTVI) {
		return HDTVI_AudioPlayBack(Chnl);
	}

	return 0;
}

int HDDVR_UTC_Init(int chnl)
{
	if(gHDChip == HDCHIP_HDAHD)
	{
		HDAHD_UTC_Init(chnl);
...
int HDDVR_UTC_Send(int Chnl, int Cmd, unsigned char value)
{
	...
	if(gHDChip == HDCHIP_HDTVI)
	{
		Revalue = HDTVI_UTC_Send(Chnl, Cmd, value);
	}
